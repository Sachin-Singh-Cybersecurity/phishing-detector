...
# from . import URLFeatureExtraction as urlfe
# from .import abx
# model_delete_signal = Signal()

@api_view(('GET',))
def run_abx(request, url):
...
"""
    ...
    if pishing_level >= 5:
        return JsonResponse("This is a Phishing Website",safe=False)
    
    return Response({"Pishing level":pishing_level})
...
Phishers may use JavaScript to show a fake URL in the status bar to users.
 To extract this feature, we must dig-out the webpage source code, 
...
"""if __name__ == "__main__":
  url = "http://get-unlimited-followers-for-instagram@addressing-cho-chairs-bedrooms.trycloudflare.com"
  features = featureExtraction(url)
  p=sum(features)
  if(p>=5):
    print('Website is Phising')
  else:
